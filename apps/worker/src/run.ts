...
export async function executeRun(job: RunJob): Promise<string> {
	...
	if (attempt.status === 'failed' && job.loginFlowId && /login|signin|sign-in|auth/i.test(attempt.finalUrl)) {
...
async function runAttempt(
	...
	const harKey = await tryUpload(`runs/${runId}/run.har.zip`, join(workDir, 'run.har.zip'), 'application/zip');
	...
	if (status === 'passed' && !traceKey) {
		status = 'errored';
		runError = 'artifact upload failed';
	}
	...
	return { runId, status, error: runError, finalUrl, traceKey, harKey, videoKey, screenshotKey };
}
